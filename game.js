...
function cambiarPantalla(id) { showScreen(id); }
...
// Usamos localStorage para que persistan entre sesiones
let gameScores = JSON.parse(localStorage.getItem('sensedScores')) || {
    color: 0, sound: 0, time: 0, fishing: 0
};

function addPoints(mode, points) {
    if (typeof points !== 'number' || isNaN(points)) return;
    gameScores[mode] = (gameScores[mode] || 0) + points;
    localStorage.setItem('sensedScores', JSON.stringify(gameScores));
    updateNavScore();
}

function updateNavScore() {
    const total = Object.values(gameScores).reduce((a, b) => a + b, 0);
    if ($("nav-score")) $("nav-score").textContent = `${Math.round(total)} pts`;
}
...
// ─── 4. SISTEMA DE AUTH (LOGIN / REGISTRO / INVITADO) ───────────────────────
...
if (btnAuthToggle) {
    btnAuthToggle.addEventListener('click', (e) => {
        ...
        if (isLoginMode) {
            ...
            $("auth-toggle-text").textContent = '¿No tienes cuenta?';
            btnAuthToggle.textContent = 'Regístrate';
        } else {
            $("auth-title").textContent = 'Crear Cuenta';
            $("btn-auth-submit").textContent = 'REGISTRARSE';
            $("auth-toggle-text").textContent = '¿Ya tienes cuenta?';
...
if (btnAuthSubmit) {
    btnAuthSubmit.addEventListener('click', async (e) => {
        ...
        if (!username || !password) { alert("Por favor, rellena todos los campos."); return; }
...
if (btnGuest) {
    btnGuest.addEventListener('click', () => {
        const guestName = 'Invitado_' + rand(1000, 9999);
        currentUser = { username: guestName, isGuest: true };
        enterGame();
    });
}

// Función compartida que abre el juego
function enterGame() {
    $("screen-auth").classList.remove('active');
    $("main-nav").style.display = 'flex';
    // Resetear puntuaciones al entrar
    gameScores = { color: 0, sound: 0, time: 0, fishing: 0 };
    localStorage.setItem('sensedScores', JSON.stringify(gameScores));
    updateNavScore();
...
document.querySelectorAll(".nav-btn").forEach(btn => {
    btn.addEventListener("click", () => {
        ...
        showScreen(screens[btn.dataset.mode] || "screen-home");
    });
});
...
function updateColorPreview() {
    const box = $("color-preview-box");
    if (box) box.style.backgroundColor = hslStr(getGuessColor());
}
...
if (btnColorConfirm) {
    btnColorConfirm.addEventListener("click", () => {
        ...
        // Sumar puntos al marcador global
        addPoints('color', Math.round(sc * 10));
    ...
    state.colorScores.forEach(({ orig, guess, sc }) => {
        ...
        pair.innerHTML = `
            <div style="display:flex; justify-content:center; gap:0.5rem; align-items:center; margin-bottom: 0.5rem;">
                <div style="background:${hslStr(orig)}; width:30px; height:30px; border-radius:4px;"></div>
                <div style="background:${hslStr(guess)}; width:30px; height:30px; border-radius:4px;"></div>
                <span style="color:var(--text); font-weight:bold;">${sc.toFixed(1)}</span>
            </div>`;
...
// ─── 7. MODO SONIDO (5 Rondas) ───────────────────────────────────────────────
let audioCtx = null, currentTargetFreq = 0, userOsc = null, userGain = null;
let soundRound = 0, soundTotalScore = 0, soundListens = 0;
...
if (btnSoundStart) {
    btnSoundStart.addEventListener("click", () => {
        ...
        soundRound = 0; soundTotalScore = 0;
        loadSoundRound();
    });
}
...
function loadSoundRound() {
    if (soundRound >= 5) {
        ...
        addPoints('sound', soundTotalScore);
    ...
    if (playBtn) { playBtn.textContent = "ESCUCHAR OBJETIVO (3/3)"; playBtn.style.opacity = "1"; playBtn.style.pointerEvents = "auto"; }
...
function startUserTone() {
    ...
    userOsc.connect(userGain); userGain.connect(audioCtx.destination); userOsc.start();
}

function stopUserTone() {
    if (userOsc) { userOsc.stop(); userOsc.disconnect(); userOsc = null; }
}

const btnSoundPlay = $("btn-sound-play");
if (btnSoundPlay) {
    btnSoundPlay.addEventListener("click", () => {
        if (soundListens >= 3) { showToast("¡Ya has usado tus 3 oportunidades!"); return; }
        const inst = instruments[soundRound];
        if (currentTargetFreq === 0) currentTargetFreq = Math.round(rand(inst.min, inst.max));
        playTargetTone(); soundListens++;
...
if (btnTimeAction) {
    btnTimeAction.addEventListener("click", () => {
        ...
        if (timeState === "waiting") {
            ...
            resultText.textContent = "Contando..."; resultText.style.color = "var(--text2)";
            ...
            if (absDiff <= 50)       { resultText.textContent = `¡PERFECTO! (${diffStr}) 🔥`;      resultText.style.color = "var(--success)"; timeTotalScore += 20; }
            else if (absDiff <= 200) { resultText.textContent = `¡Muy preciso! (${diffStr}) ⚡`;   resultText.style.color = "var(--accent)";  timeTotalScore += 10; }
            else                     { resultText.textContent = `Demasiado lejos (${diffStr}) 💀`; resultText.style.color = "var(--danger)"; }
            timeRound++;
...
// ─── 9. MODO PESCA (5 Lances) ────────────────────────────────────────────────
let fishState = "idle", fishTimeout = null, biteStartTime = 0;
let fishRound = 0, fishTotalScore = 0;

const btnFishingStart = $("btn-fishing-start");
if (btnFishingStart) {
    btnFishingStart.addEventListener("click", () => {
        fishRound = 0; fishTotalScore = 0;
...
function resetFishing() {
    if (fishRound >= 5) {
        ...
        addPoints('fishing', fishTotalScore);
...
function getRandomCatch(reactionTime) {
    ...
    if (reactionTime <= 260) {
        tierColor = "var(--success)"; tierMsg = "¡CLAVADÓN PERFECTO! 🔥";
        pool = [
        ...
        tierColor = "var(--accent)"; tierMsg = "¡Buena clavada! ⚡";
        pool = [
        ...
        tierColor = "var(--text)"; tierMsg = "Por los pelos... 💧";
        pool = [
...
if (btnFishAction) {
    btnFishAction.addEventListener("click", () => {
        const btn = $("btn-fish-action"), resultText = $("fish-result-text");
        if (fishState === "idle") {
            fishState = "waiting";
            btn.style.backgroundColor = "var(--bg3)"; btn.style.color = "var(--text)";
            ...
            fishTimeout = setTimeout(() => {
                fishState = "bite"; biteStartTime = Date.now();
            ...
            fishRound++; setTimeout(resetFishing, 2000);
            ...
            fishRound++; setTimeout(resetFishing, 3000);
        }
    });
}
...
window.addEventListener("keydown", (e) => {
    if (e.code === "Space") {
        ...
        if (timeGame    && timeGame.classList.contains("active"))    { e.preventDefault(); $("btn-time-action").click(); }
        else if (fishingGame && fishingGame.classList.contains("active")) { e.preventDefault(); $("btn-fish-action").click(); }
    }
});
